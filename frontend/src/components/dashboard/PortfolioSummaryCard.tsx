...
export default function PortfolioSummaryCard({ totalValue, totalCost, totalCash, gainLoss, gainLossPct, realizedPnl, currency }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-bg-card p-6 border border-border-subtle card-hover">
      ...
      <div className="mt-4 flex items-center gap-6 flex-wrap">
        ...
        <div>
          <p className="text-xs text-t-faint">浮动盈亏</p>
        ...
        <div>
          <p className="text-xs text-t-faint">已实现盈亏</p>
          <div className={`flex items-center gap-1 text-sm font-medium ${isRealizedGain ? 'text-gain' : 'text-loss'}`}>
            {isRealizedGain ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
            <span>{isRealizedGain ? '+' : ''}{formatCurrency(realizedPnl, currency)}</span>
          </div>
        </div>
      </div>
    </div>
