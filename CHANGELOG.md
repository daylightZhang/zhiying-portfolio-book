...
### 优化
- **建仓时自动获取行情**: 创建持仓后若 MarketQuote 表无该 symbol 缓存，立即调 `refresh_single_price` 拉一次价格
  - 同步执行，建仓返回时 `current_price` 已带上
  - 失败仅 warning，不影响建仓本身
  - 关联持仓和普通持仓都覆盖

---

## 2026-05-29 v1.11.5 - 已实现盈亏支持时间过滤

### 新增
- **已实现盈亏时间过滤**: 后端 `/portfolio/summary` 支持 `realized_start` / `realized_end` 过滤 SELL 事件区间
  - 成本基准仍按全量交易回放（保证移动加权平均正确），仅在累加 realized 时按区间筛选
  - 前端默认本年度（1/1 ~ 今天），快捷按钮：今年 / 去年 / 近 12 月 / 全部 / 自定义日期区间
  - 过滤同时影响顶部"已实现盈亏"汇总和明细表，口径一致
  - 区间内无已实现盈亏时显示空态提示
...
### 优化
- **券商持仓选择器**: 替换原生 `<select>` 为自定义 `BrokerPositionSelect` 组件
  - 顶部搜索框：支持按代码/名称/账户名实时过滤
  - 每个选项展示：市场色块徽章 + 股票代码（等宽字体）+ 名称 + 持仓数量
  - 按券商账户分组，组头有 Briefcase 图标 + 大写小标签
  - 玻璃拟态面板、平滑入场动画，统一项目主题
  - 已选项在触发按钮中以同样的紧凑徽章样式呈现，告别原生灰白下拉
...
- **美股IPO监测页面**: 侧边栏新增"美股IPO"菜单项
  - 数据源: moomoo.com 新股中心页面爬取 (Playwright 无头浏览器)
  - "已上市" / "待上市" Tab 切换，已上市从 SSR 获取，待上市通过模拟点击 Tab 触发 XHR
  - 展示: 代码、股票名称、上市日期、价格、发行价、首日涨幅、累计涨幅、行业
  - 按上市日期从近到远排列
  - 后端 1 小时缓存 TTL，Playwright 失败时回退到简单 HTTP 爬取
